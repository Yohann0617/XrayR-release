...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/Yohann0617/XrayR-release/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
install_bbr() {
    bash <(curl -L -s https://raw.githubusercontent.com/Yohann0617/Linux-NetSpeed/master/tcp.sh)
    #if [[ $? == 0 ]]; then
    #    echo ""
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/Yohann0617/XrayR-release/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/Yohann0617/XrayR ---
  ${green}0.${plain} 修改配置
————————————————
